...
# ── 6 ter. Carte Choroplète – Normandie ─────────────────────────────────────────────
...
# ── 7. TOP 3 COMMUNES PAR DÉPARTEMENT – ANALYSE D'INVESTISSEMENT ────────────────
...
# ── 8. Évolution régionale : Maisons vs Appartements ─────────────────────────
# Objectif (2) – slide 4 : courbes temporelles Index_{c,t}
# Ici on agrège à l'échelle de toute la Normandie (médiane des log_net)
# pour obtenir un benchmark régional unique par type de bien.

import matplotlib.ticker as mticker

# ── 8a. Calcul du benchmark régional (toute la Normandie) ────────────────────
def benchmark_regional(indices: dict) -> pd.DataFrame:
    """
    Pour chaque type de bien (Maison / Appartement), calcule :
        log_net_regional(t) = médiane { log_price_net_i : t(i) = t }
        Index_regional(t) = 100 × exp( log_net_regional(t) − log_net_regional(t₀) )
    Retourne un DataFrame long avec colonnes : annee_mois, periode, type_bien, index_regional
    """
    records = []
    for type_bien, data in indices.items():
        bench = data["bench"].copy()

        # Médiane régionale = médiane des benchmarks départementaux pondérée par le nb de communes
        # (approche simple : médiane des log_net sur toutes les périodes)
        panel = data["panel"].copy()

        reg = (panel.groupby("periode")["log_net"]
               .median()
               .reset_index()
               .rename(columns={"log_net": "log_net_reg"}))
        reg["annee_mois"] = pd.to_datetime(reg["periode"])
        reg = reg.sort_values("annee_mois").reset_index(drop=True)

        # Référence t₀ [slide 17]
        ref_row = reg[reg["periode"] == REFERENCE_PERIODE]
        log_ref = ref_row["log_net_reg"].values[0] if not ref_row.empty else reg["log_net_reg"].iloc[0]

        # Index_{regional, t} = 100 × exp(log_net_reg(t) − log_ref)
        reg["index_regional"] = 100 * np.exp(reg["log_net_reg"] - log_ref)
        reg["type_bien"] = type_bien
        records.append(reg)

    return pd.concat(records, ignore_index=True)


df_reg = benchmark_regional(indices)

# ── 8b. Visualisation : courbe comparative Maisons vs Appartements ────────────
fig, axes = plt.subplots(1, 2, figsize=(16, 6))
fig.suptitle(
    rf"Évolution régionale des prix — Normandie"
    f"\n(base 100 = {REFERENCE_PERIODE}  |  Méthodologie hédonique, slide 17)",
    fontsize=13, fontweight="bold"
)

COLORS = {"Maison": "#E65100", "Appartement": "#1565C0"}
STYLES = {"Maison": "-",       "Appartement": "--"}

# ── Graphique gauche : indices superposés ──────────────────────────────────────
ax = axes[0]
for type_bien, grp in df_reg.groupby("type_bien"):
    grp = grp.sort_values("annee_mois")
    ax.plot(
        grp["annee_mois"], grp["index_regional"],
        label=type_bien,
        color=COLORS[type_bien],
        linestyle=STYLES[type_bien],
        lw=2.2
    )

ax.axhline(100, color="grey", ls=":", lw=1, label=f"Base 100 ({REFERENCE_PERIODE})")
ax.set_title("Index régional — Maisons vs Appartements")
ax.set_ylabel(r"$\mathrm{Index}_{t}$ (base 100)")
ax.legend(fontsize=10)
ax.grid(axis="y", alpha=0.3)
ax.yaxis.set_major_formatter(mticker.FormatStrFormatter("%.0f"))

# ── Graphique droit : écart relatif Maisons − Appartements ────────────────────
ax2 = axes[1]

# Pivot pour calculer le spread
pivot_reg = df_reg.pivot_table(index="annee_mois", columns="type_bien", values="index_regional")
pivot_reg = pivot_reg.sort_index()

if "Maison" in pivot_reg.columns and "Appartement" in pivot_reg.columns:
    pivot_reg["spread"] = pivot_reg["Maison"] - pivot_reg["Appartement"]
    ax2.plot(pivot_reg.index, pivot_reg["spread"], color="#6A1B9A", lw=2)
    ax2.fill_between(
        pivot_reg.index,
        0, pivot_reg["spread"],
        where=pivot_reg["spread"] >= 0,
        alpha=0.15, color="#E65100", label="Maisons > Apparts"
    )
    ax2.fill_between(
        pivot_reg.index,
        0, pivot_reg["spread"],
        where=pivot_reg["spread"] < 0,
        alpha=0.15, color="#1565C0", label="Appartements > Maisons"
    )
    ax2.axhline(0, color="grey", ls=":", lw=1)
    ax2.set_title("Écart d'indice (Maisons − Appartements)")
    ax2.set_ylabel("Différence de points d'indice")
    ax2.legend(fontsize=9)
    ax2.grid(axis="y", alpha=0.3)

plt.tight_layout()
plt.savefig("outputs/evolution_regionale_maison_vs_appart.png", dpi=150, bbox_inches="tight")
plt.show()
print("Sauvegardé → outputs/evolution_regionale_maison_vs_appart.png")

# ── 8c. Tableau récapitulatif régional ────────────────────────────────────────
print(f"\n{'='*65}")
print(f"  BENCHMARK RÉGIONAL NORMANDIE — Maisons vs Appartements")
print(f"{'='*65}")
print(f"{'Type':<16}{'Index fin':<14}{'Variation (%)':<16}{'CAGR (% / an)'}")
print("─" * 65)

for type_bien, grp in df_reg.groupby("type_bien"):
    grp = grp.sort_values("annee_mois")
    idx_0 = grp["index_regional"].iloc[0]
    idx_T = grp["index_regional"].iloc[-1]
    n_years = max((grp["annee_mois"].iloc[-1] - grp["annee_mois"].iloc[0]).days / 365.25, 0.5)
    variation = (idx_T / idx_0 - 1) * 100
    cagr      = ((idx_T / 100) ** (1 / n_years) - 1) * 100
    print(f"  {type_bien:<14}{idx_T:<14.1f}{variation:+<16.1f}{cagr:+.2f}%")
