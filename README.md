...
| **Heatmap commune × temps** | Dynamiques, ruptures et hétérogénéité spatiale |
...
# Typologies T (caractéristiques du bien xᵢ)
...
> `drop_first=True` retire une modalité par groupe pour éviter la multicolinéarité parfaite, conformément aux slides 10–11.
...
# Interpolation : linéaire → ffill → bfill  [slide 16]
...
| Stabilité | $1 / \text{vol}(\Delta \log \text{Index}_{c,t})$ | 25 % |
...
def score_communes(panel, type_bien):
    ...
    # Stabilité = inverse de la volatilité mensuelle
...
① Nettoyage : yᵢ = log(price_sqm)
      │
      ▼
② Dummies : γ_t, δ_d, μ_c, θ_{d,a}, x_i
      │
      ▼
